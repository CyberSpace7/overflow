...
var GridLayout = React.createClass({
  ...
  onAddItem() {
    ...
    if (init === 1) {
      ...
      this.hideDefault();
      this.refs.gridedittoolbar.setState({showWizard: false})
    }
  },
  ...
  editSave: function() {
    ...
    this.refs.gridedittoolbar.refs.saveconfirm.show();
  },

  editModeToggle: function () {
    var wizState = this.state.showWizard;
    console.log('curState: ' + this.state.editMode)
    if (this.state.editMode == false){
      console.log('Edit Mode: true');
      // this.props.onChildStateChange(true, wizState);
      this.setState({editMode: true, showWizard: wizState, isDraggable: true, isResizable: true});
      this.itemOpts();
    }
    if (this.state.editMode == true){
      console.log('Edit Mode : false');
      // this.props.onChildStateChange(false, wizState);
      this.setState({editMode: false, showWizard: wizState, isDraggable: false, isResizable: false});
      this.itemOpts();
    }
      return null
  },
  ...
  render() {
    ...
    return (
      <div style={{margin: 10 + 'px'}}>
        <GridEditToolbar ref="gridedittoolbar" onChildStateChange={this.onChildStateChange} onAddItem={this.onAddItem} editModeToggle={this.editModeToggle} editMode={this.state.editMode} editSave={this.editSave}/>
